...
const AddressGrid: React.FC<{ address?: any; lang: string }> = ({ address, lang }) => {
  ...
  return (
    <div className='AddressCheckout flex justify-between h-full -mt-4 text-15px md:mt-0'>
      <RadioGroup
        value={selected}
      ...
      >
        ...
        {address?.length > 0 ? (
          address?.map((item: any, index: any) => (
            ...
            >
              <RadioGroup.Label as='h3' className='mb-2 -mt-1 font-semibold text-brand-dark '>
                {item?.title}
              </RadioGroup.Label>
              <RadioGroup.Description as='div' className='leading-6 text-brand-muted'>
